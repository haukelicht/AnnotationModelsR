...
#'    }
create_em_bin_indixes <- function(x, .echo) {
...
#'
#' @param x a list as returned by \code{\link{create_em_bin_indixes}}
#' @param .ability.prior non-negative number, specifying the annotator ability prior used to initialize ability parameters for first iteration
#'      Defaults to 0.7
#'
#' @return a list with elements
#'     \itemize{
...
#'     }
init_em_bin_params <- function(x, .ability.prior){
...
#'
#' @param p a list of initialized parameters as returned by \code{\link{init_em_bin_params}}
#' @param max.iters
#' @param ability.prior non-negative number, specifying the annotator ability prior used to initialize ability parameters for first iteration
#' @param beta.prior non-negative number, specifying the beta prior used to perform smoothing on \eqn{\hat{pi}} (label class prevalence estimates)
#' @param alpha.prior non-negative number, specifying the alpha prior used to perform smoothing on items' label class probability estimates
#' @param min.relative.diff non-negative number, specifying the minimum relative difference between log-likelihoods of two subsequent iteration steps.
#'      Governs when convergence is reached (the lower, the more iterations).
#'      Defaults to 1e-8
#' @describeIn create_em_bin_indixes
#'
#' @return a list with elements
#'     \itemize{
#'       \item 'theta_hat': annotator parameter matrices (a 3d-tensor with extension \eqn{K* K* J})
...
#'     }
em_bin_iterate <- function(x, p, max.iters, beta.prior, alpha.prior, min.relative.diff, .echo) {
...
#'           \item 'prop_labels' (the label proportions, where labels are induced by applying a .5 threshold to posterior label class probability estimates), and
...
#'           \item 'labeled' (\eqn{y_{ij}}, the label annotator \eqn{j} assigns to item \eqn{i}), and
...
em <- function(
  ...
  dm <- data %>%
    ungroup() %>%
    dplyr::mutate(
      `_item` = dplyr::group_indices(., !!enquo(item.col)) ,
      `_annotator` = dplyr::group_indices(., !!enquo(annotator.col)),
      `_label` = dplyr::group_indices(., !!enquo(label.col))
    )

  # create item, annotator and label indexes
  dat <- create_em_bin_indixes(dm, echo)

  # Estimation:

  # initialize parameters
  prms <- init_em_bin_params(dat, .ability.prior)
  ...
  majority_votes <- dm %>%
    dplyr::group_by(`_item`, !!enquo(item.col), !!enquo(label.col)) %>%
    dplyr::summarize(votes = n_distinct(!!enquo(annotator.col))) %>%
    dplyr::group_by(`_item`, !!enquo(item.col), !!enquo(label.col), votes) %>%
    dplyr::mutate(
      tie_breaker = runif(1),
      majority_vote = !!enquo(label.col)
    ) %>%
    dplyr::group_by(!!enquo(item.col)) %>%
    dplyr::filter(votes == max(votes)) %>%
    dplyr::slice(which.max(tie_breaker)) %>%
    dplyr::select(!!enquo(item.col), majority_vote) %>%
    ungroup()
  ...
  z_out <- dplyr::tibble(
    ...
    dplyr::select(!!enquo(item.col), !!enquo(label.col), est_prob) %>%
    tidyr::spread(2, est_prob) %>%
    dplyr::left_join(majority_votes, by = as.character(args$item.col))
  ...
  pi_out <- dplyr::tibble(
    ...
    dplyr::select(!!enquo(label.col), est_prob, prop_labels) %>%
    dplyr::left_join(
      prop.table(table(z_out$majority_vote)) %>%
        tibble::enframe() %>%
        dplyr::rename(!!enquo(label.col) := name, prop_mv = value)
      , by = as.character(args$label.col)
    )

  # annotator ability estimats
  theta_out <- dplyr::tibble(
    ...
    dplyr::rename(labeled := !!enquo(label.col)) %>%
    dplyr::left_join(
      unique(select(dm, !!enquo(label.col), `_label`))
      , by = c("cat_label" = "_label")
    ) %>%
    dplyr::select(!!enquo(annotator.col), !!enquo(label.col), labeled, est_prob)
